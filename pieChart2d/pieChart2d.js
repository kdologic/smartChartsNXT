...
window.SmartChartsNXT.PieChart = function(opts)
{
  ...
  function init()
  {
    ...
    if(PAGE_OPTIONS.showLegend){
      if(PAGE_OPTIONS.width <= 480){
        PAGE_DATA.pieWidth=PAGE_DATA.pieHeight=Math.min((PAGE_OPTIONS.width*7/10)/2,(PAGE_OPTIONS.height-200))*30/100;
  ...
  function createPie(startAngle,endAngle,color,index)
  {
    ...
    strSVG += "  <path class='pie"+index+"'  id='upperArcPie"+index+"'  fill='"+color+"' stroke='#eee' stroke-width='"+(PAGE_OPTIONS.outline||1)+"' style='cursor:pointer;' \/>";
      ...
      function callChart()
      {
        if(containerDiv){
          if(containerDiv.offsetWidth === 0 && containerDiv.offsetHeight === 0){
            timeOut = setTimeout(function(){callChart();},100);
          }else {
            timeOut = setTimeout(function(){init();},500);
          }
        }
      }
    }
