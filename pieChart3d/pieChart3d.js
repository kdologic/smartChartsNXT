    ...
    try {
      ...
      for(var pieIndex = 0;pieIndex < PAGE_DATA.uniqueDataSet.length;pieIndex++)
      {
        function onClick(e){
          ...
          var pieData,elemId = e.target.className.baseVal,sliceOut;
    ...
    try {
      ...
      for(var pieIndex = 0;pieIndex < PAGE_DATA.uniqueDataSet.length;pieIndex++)
      {
        ...
        for(var i=0;i<pieElems.length;i++)
        {
          ...
          var lowerArcTriangle = [],arrLowerArcPath = document.querySelector("#"+PAGE_OPTIONS.targetElem+" #pieChart3D #lowerArcPie"+pieIndex).getAttribute("d").split(" ");
          ...
          {
            var polygonA = [],arrSideBPath = pieElems[i].getAttribute("d").split(" ");
          ...
          {
            var polygonA = [],arrSideAPath = pieElems[i].getAttribute("d").split(" ");
          ...
          if(pieElems[i].getAttribute("id").match("upperArcPie"))
            renderingOrder.upperArc.push({elemId:pieElems[i].getAttribute("id"),distBottom:distanceBottom,distTop:distanceTop});
          else if(pieElems[i].getAttribute("id").match("lowerArcPie"))
            renderingOrder.lowerArc.push({elemId:pieElems[i].getAttribute("id"),distBottom:distanceBottom,distTop:distanceTop});
          else if(pieElems[i].getAttribute("id").match("sideA") || pieElems[i].getAttribute("id").match("sideB"))
            renderingOrder.sideArc.push({elemId:pieElems[i].getAttribute("id"),distBottom:distanceBottom,distTop:distanceTop,"isInside":isInside});
          else if(pieElems[i].getAttribute("id").match("sideC"))
            renderingOrder.sideArc.push({elemId:pieElems[i].getAttribute("id"),distBottom:distanceBottom,distTop:distanceTop});
        }
      }
