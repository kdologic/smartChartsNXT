...
(function(){
  ...
  window.PieChart2D = function(opts)
  {
    ...
    function init()
    {
      ...
      //var svgWidth = parseInt($("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").attr("width"));
      //var svgHeight = parseInt($("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").attr("height"));
    ...
    function prepareChart()
    {
      ...
      prepareDataSet();
      console.log(PAGE_DATA.scaleX);
      console.log(PAGE_DATA.scaleY);
      var strSVG="";
      ...
      strSVG += "  <\/text>";
      strSVG += "<\/g>";
      //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").append(strSVG);
      ...
      //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#txtTitleGrp").find("#txtTitle").text(PAGE_OPTIONS.title);
      //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#txtSubtitle").text(PAGE_OPTIONS.subTitle);
      ...
      //makeUnselectable($("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("text"));
      //for(var pieIndex = 0;pieIndex < PAGE_DATA.uniqueDataSet.length;pieIndex++)
        //makeUnselectable($("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find(".pie"+pieIndex));
    ...
    function createPie(startAngle,endAngle,color,index)
    {
      ...
      strSVG += "  <text class='pie"+index+"' id='txtPercentPie"+index+"' fill='#717171' font-family='Lato' fill='#000' style='opacity:0.5;' >"+percent+"%</text>";
      ...
      //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#lowerArcPie"+index).attr("d",lowerArcPath.d).attr("fill","url(#gradRadialPie"+index+")");
      //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#sideAPie"+index).attr("d", describeRect(upperArcPath.center, upperArcPath.start,lowerArcPath.start, lowerArcPath.center));
      //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#sideBPie"+index).attr("d", describeRect(upperArcPath.center ,upperArcPath.end,lowerArcPath.end, lowerArcPath.center));
      //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#sideCPie"+index).attr("d", describeArcFill(upperArcPath, lowerArcPath));//.attr("fill","url(#gradLinearPie"+index+")");
      var textLabel = PAGE_DATA.uniqueDataSet[index]["label"];
      textLabel = (textLabel.length > 20?textLabel.substring(0,20)+"...":textLabel);
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #pieChart2D #txtPie"+index).textContent = (textLabel+", "+PAGE_DATA.uniqueDataSet[index]["value"]);
        ...
        for(var pieIndex = 0;pieIndex < PAGE_DATA.uniqueDataSet.length;pieIndex++)
        {
          ...
          function onClick(e){
            ...
            var shiftInterval = setInterval(function()
            {
              ...
              //var lowerArcPath = describeEllipticalArc(shiftedCentre.x,(shiftedCentre.y+PAGE_DATA.pieThickness),PAGE_DATA.pieWidth,PAGE_DATA.pieHeight,pieData["lowerArcPath"].startAngle,pieData["lowerArcPath"].endAngle,0);
              if(shiftIndex === 0)
               upperArcPath = pieData.upperArcPath;
              ...
              //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#sideAPie"+index).attr("d", describeRect(upperArcPath.center, upperArcPath.start,lowerArcPath.start, lowerArcPath.center));
              //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#sideBPie"+index).attr("d", describeRect(upperArcPath.center ,upperArcPath.end,lowerArcPath.end, lowerArcPath.center));
              //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#sideCPie"+index).attr("d", describeArcFill(upperArcPath, lowerArcPath));
    ...
    function divergeElements(){
      if(PAGE_DATA.uniqueDataSet.length >1){
        ...
        { 
          ...
          $("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find(".pie"+pieIndex).css({"transform": "translate("+(transposePoint.x-PAGE_DATA.pieCenter.x)+"px,"+(transposePoint.y-PAGE_DATA.pieCenter.y)+"px)"});
        }
      }
    }/*End divergeElemnts()*/
          ...
          { 
            ...
            setTimeout(function()
            {
              var intervalId = setInterval(function()
              {
                ...
                $("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find(".pie"+pieIndex).css({"transform": "translate("+transform.x+"px,"+transform.y+"px)"});
                ...
                if(Math.round(transform.x) === 0 && Math.round(transform.y) === 0){
                  ...
                  $("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find(".pie"+pieIndex).css({"transform": ""});
                }
              },5);
            },(pieIndex*400));
    ...
    function rotateChart(rotationIndex,ignorOrdering)
    {
      for(var pieIndex = 0;pieIndex < PAGE_DATA.uniqueDataSet.length;pieIndex++)
      {
        ...
        //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#upperArcPie"+pieIndex).attr("d",upperArcPath.d).attr("fill","url(#gradRadialPie"+pieIndex+")");
        //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#lowerArcPie"+pieIndex).attr("d",lowerArcPath.d).attr("fill","url(#gradRadialPie"+pieIndex+")");
        //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#sideAPie"+pieIndex).attr("d", describeRect(upperArcPath.center, upperArcPath.start,lowerArcPath.start, lowerArcPath.center));
        //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#sideBPie"+pieIndex).attr("d", describeRect(upperArcPath.center ,upperArcPath.end,lowerArcPath.end, lowerArcPath.center));
        //$("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#sideCPie"+pieIndex).attr("d", describeArcFill(upperArcPath, lowerArcPath));
    ...
    function resetOrdering()
    {
      try {
        ...
        for(var pieIndex = 0;pieIndex < PAGE_DATA.uniqueDataSet.length;pieIndex++)
        {
          ...
          var pieElems = document.querySelectorAll("#"+PAGE_OPTIONS.targetElem+" #pieChart2D .pie"+pieIndex);
          for(var i=0;i<pieElems.length;i++)
          {
            ...
            //createDot(arcBottom,getColor(i));
            //createDot(arcTop,getColor(i));
        ...
        {
          var elem = $("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#"+elemId);
          $("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").find("#"+elemId).remove();
          $("#"+PAGE_OPTIONS.targetElem).find("#pieChart2D").append(elem);
        }
    ...
    {
      //var titleBoundBox = document.querySelector("#"+PAGE_OPTIONS.targetElem+" #pieChart2D #txtTitleGrp #txtTitle").getBBox();
      ...
      for(var pieIndex = 0;pieIndex < PAGE_DATA.uniqueDataSet.length;pieIndex++)
      {
        ...
        var txtBoundingRect = document.querySelector("#"+PAGE_OPTIONS.targetElem+" #pieChart2D #txtPie"+pieIndex).getBoundingClientRect();//.getBBox();
        
        var txtContainer = document.querySelector("#"+PAGE_OPTIONS.targetElem+" #pieChart2D #txtContainer"+pieIndex);
        ...
        var txtPercent = document.querySelector("#"+PAGE_OPTIONS.targetElem+" #pieChart2D #txtPercentPie"+pieIndex);
        txtPercent.setAttribute("x",textPos.x-(txtBoundingRect.width/2));
        txtPercent.setAttribute("y",textPos.y+(txtBoundingRect.height/2)+25);
        txtPercent.setAttribute("width",txtBoundingRect.width);
        txtPercent.setAttribute("height",5);
      }
    }/*End resetTextPos()*/
