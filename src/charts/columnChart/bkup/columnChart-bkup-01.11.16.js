...
function prepareDataSet(dataSet)
  ...
  function createColumns(dataSet,index,scaleX)
  {
    ...
    for(var dataCount = 0;dataCount<dataSet.length;dataCount++)
    {
      ...
      var fill = color,filter = "";
      switch(PAGE_OPTIONS.dataSet.series.gradient){
        case "oval":
          appendGradFillOval(index);
          fill = "url(#"+PAGE_OPTIONS.targetElem+"-columnchart-gradOval"+index+")";
  ...
  function appendGradFillOval(index) {
    /*Creating gradient fill for area*/
    var stopColor = PAGE_OPTIONS.dataSet.series[index].color||$SC.util.getColor(index);
    var strSVG = "";
    strSVG += "<defs>";
    
    strSVG += "<linearGradient  id='"+PAGE_OPTIONS.targetElem+"-columnchart-gradOval"+index+"' x1='0%' y1='0%' x2='100%' y2='0%'>  ";
