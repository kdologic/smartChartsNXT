...
window.SmartChartsNXT.DonutChart = function(opts)
{
  ...
  function init()
  {
    ...
    if(PAGE_OPTIONS.showLegend)
      PAGE_DATA.donutCenter = new $SC.geom.Point(PAGE_DATA.svgCenter.x-150,PAGE_DATA.svgCenter.y+50) ;
    else
  ...
  function createLegands(){
    ...
    var legendMargin = {left:220,top:50};
    var lengendBBox = {
      left:(PAGE_DATA.donutCenter.x+PAGE_DATA.donutWidth+legendMargin.left),
      top:(PAGE_DATA.donutCenter.y-PAGE_DATA.donutHeight)
    } ;
    ...
    for(var index=0;index<PAGE_DATA.uniqueDataSet.length;index++){
      ...
      strSVG +="<rect id='legend_color_"+index+"' class='donut_legend"+index+"' x='"+lengendBBox.left+"' y='"+(lengendBBox.top+(index*30))+"' width='15' height='15' fill='"+$SC.util.getColor(index)+"' stroke='none' stroke-width='1' opacity='1'></rect>";
      strSVG +="<text id='legend_txt_"+index+"' class='donut_legend"+index+"' font-size='16' x='"+(lengendBBox.left+20)+"' y='"+(lengendBBox.top+(index*30)+14)+"' fill='#717171' font-family='Lato' >"+PAGE_DATA.uniqueDataSet[index]["label"]+"</text>";
      strSVG +="<text id='legend_value_"+index+"' class='donut_legend"+index+"' font-size='16' x='"+(lengendBBox.left+180)+"' y='"+(lengendBBox.top+(index*30)+14)+"' fill='#717171' font-family='Lato' >"+PAGE_DATA.uniqueDataSet[index]["value"]+"</text>";
      strSVG +="</g>";
    }
      ...
      for(var donutIndex = 0;donutIndex < PAGE_DATA.uniqueDataSet.length;donutIndex++)
      {
        ...
        if(PAGE_OPTIONS.showLegend){
          ...
          donutLegend.addEventListener("mouseover", function(e){
            ...
            upperArcDonut = document.querySelector("#"+PAGE_OPTIONS.targetElem+" #donutChart #upperArcDonut"+donutIndex);
            var donutData;
