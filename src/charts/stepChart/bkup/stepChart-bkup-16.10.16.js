  ...
  function init()
  {
    try {
      ...
      if(PAGE_OPTIONS.zoomWindow){
        ...
        PAGE_DATA.windowRightIndex = (longSeriesLen*2)-1;
console.log(PAGE_DATA.windowLeftIndex,PAGE_DATA.windowRightIndex,longSeriesLen);
      prepareChart();
  ...
  function prepareChart()
  {
    ...
    for(var i=0;i<PAGE_DATA.fullSeries[PAGE_DATA.longestSeries].length/2;i++)
  ...
  function createSeries(dataSet,index,scaleX)
  {
    ...
    for(var dataCount = 0;dataCount<dataSet.length;dataCount++)
    {
      var p1 = new $SC.geom.Point(PAGE_DATA.marginLeft+(dataCount*interval),((PAGE_DATA.marginTop+PAGE_DATA.gridBoxHeight)-(dataSet[dataCount]*scaleY)));
      var p2 = new $SC.geom.Point(PAGE_DATA.marginLeft+(dataCount*interval)+(interval),((PAGE_DATA.marginTop+PAGE_DATA.gridBoxHeight)-(dataSet[dataCount]*scaleY)));
      if(dataCount === 0)
        d.push("M");
      else
        d.push("L");
    ...
    strSeries +="<path stroke='"+color+"' fill='none' d='"+d.join(" ")+"' stroke-width='2' opacity='1'></path>";
    strSeries += "</g>";
    ...
    if(dataSet.length > 10){
      ...
      strSeries +="<path id='area_"+index+"' fill='"+color+"' d='"+area.join(" ")+"' stroke-width='2' stroke='none'opacity='0.1'></path>";
    }else {
    ...
    try{
      ...
      { 
        ...
        if(toolTipPoint)
        {
          ...
          toolTipRow2=(PAGE_OPTIONS.dataSet.yAxis.title+" "+(PAGE_OPTIONS.dataSet.yAxis.prefix||"")+" "+PAGE_DATA.newDataSet[indx][Math.ceil((npIndex-1)/2)]);
