...
window.SmartChartsNXT.StepChart = function (opts) {
  ...
  var PAGE_CONST = {
    ...
    MIN_HEIGHT: 400,
    hGridCount: 9,
    runId: "stepchart_" + Math.round(Math.random() * 1000000001)
  };
  ...
  function createSeries(dataSet, index, scaleX) {
    ...
    var areaOpacity = PAGE_OPTIONS.dataSet.series[index].areaOpacity || 0.2;
  ...
  function createLegands(index) {
    var seriesLegend = document.querySelector("#" + PAGE_OPTIONS.targetElem + " #stepChart #legendContainer #series_legend_" + index);
  ...
  function showAnimatedView() {
    ...
    for (var i = 0; i < PAGE_OPTIONS.dataSet.series.length; i++) {
      var set = {
        "data": PAGE_OPTIONS.dataSet.series[i].data.slice(Math.ceil(PAGE_DATA.windowLeftIndex / 2), Math.ceil((PAGE_DATA.windowRightIndex) / 2))
      };
      dataSet.push(set);
    }
