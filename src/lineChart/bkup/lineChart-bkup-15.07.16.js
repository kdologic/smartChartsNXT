 ...
 * @Sample caller code:
  ...
  SmartCharts.ready(function(){
    var lineChart = new SmartCharts.LineChart(settings);
  });

 */

window.SmartCharts.LineChart = function(opts)
{
  var PAGE_OPTIONS = { };
  ...
  function prepareChart()
  {
    ...
    var longestFullSeries = document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #fullSeries_"+PAGE_DATA.longestSeries);
  ...
  function createSeries(dataSet,index,scaleX)
  {
    ...
    if(dataSet.length > 1){
      for(var dataCount = 0;dataCount<dataSet.length;dataCount++)
      {
        var p = new $SC.geom.Point(PAGE_DATA.marginLeft+(dataCount*scaleX)+(interval/2),((PAGE_DATA.marginTop+PAGE_DATA.gridBoxHeight)-(dataSet[dataCount]*scaleY)));
        if(dataCount === 0)
          d.push("M");
        else
          d.push("L");
    ...
    strSeries +="<path stroke='"+$SC.util.getColor(index)+"' fill='none' d='"+d.join(" ")+"' stroke-dasharray='1,1' stroke-width='1' opacity='1'></path>";
    strSeries += "</g>";
    ...
    //area.push.apply(area,d);  
    strSeries +="<path id='line_"+index+"' filter='"+$SC.ui.dropShadow(PAGE_OPTIONS.targetElem)+"' stroke='"+$SC.util.getColor(index)+"' fill='none' d='"+line.join(" ")+"' stroke-width='2' opacity='1'></path>";
    //strSeries +="<path id='area_"+index+"' stroke='none' fill='url(#gradLinear"+index+")' d='"+area.join(" ")+"' stroke-width='1' opacity='1'></path>";
    if(dataSet.length <50){
      for(var point = 0;(point)<arrPointsSet.length;point++)
      {
        strSeries += "<circle cx="+arrPointsSet[point].x+" cy="+arrPointsSet[point].y+" r='4' class='inner-dot' style='fill:"+$SC.util.getColor(index)+"; opacity: 1; stroke-width: 1px;'></circle>";
        strSeries += "<circle cx="+arrPointsSet[point].x+" cy="+arrPointsSet[point].y+" r='2' class='outer-dot' style='fill:white; opacity: 1; stroke-width: 1px;'></circle>";
      }
    }
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart").insertAdjacentHTML("beforeend",strSeries);
  ...
  function showAnimatedView(){
    ...
    var timoutId = setInterval  (function(){
      ...
      if(pointIndex === PAGE_OPTIONS.dataSet.series[PAGE_DATA.longestSeries].data.length){
        ...
        $SC.ui.appendMenu(PAGE_OPTIONS.targetElem,PAGE_DATA.svgCenter);
      }
      pointIndex++;
    },50);
  ...
  function bindEvents()
  {
    ...
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart").removeEventListener("mousemove",onMouseMove);
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart").addEventListener("mousemove", onMouseMove,false);

    for(var index=0;index<PAGE_OPTIONS.dataSet.series.length;index++)
    {
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #series_legend_"+index).addEventListener("click", function(e){
        var arrSeries = document.querySelectorAll("#"+PAGE_OPTIONS.targetElem+" #lineChart .series");
        for(var i=0;i<arrSeries.length;i++)
          arrSeries[i].style.visibility = "hidden";
        $SC.ui.toolTip(PAGE_OPTIONS.targetElem,"hide");
        document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #series_"+e.target.id.split("_")[2]).style.visibility = "visible";
        document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #series_actual_"+e.target.id.split("_")[2]).style.visibility = "visible";
      },false);
    }
  }/*End bindEvents()*/
  ...
  {  
    try{
      ...
      var gridBox = document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #gridBox").getBBox();
      ...
      {  
        ...
        document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #vLine").setAttribute("d",vLine.join(" "));
        ...
        if(toolTipPoint)
        {
          ...
          $SC.geom.createDot(toolTipPoint,$SC.util.getColor(indx),3,1,"tooTipPoint","lineChart");
          $SC.geom.createDot(toolTipPoint,$SC.util.getColor(indx),8,0.2,"tooTipPoint","lineChart");
        ...
        $SC.ui.toolTip(PAGE_OPTIONS.targetElem,"hide");
        document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #vLine").style.display = "none";
  ...
  {
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideLSel").addEventListener("mousedown", function(e){
      e.stopPropagation();
      PAGE_DATA.mouseDown = 1; 
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideLSel").addEventListener("mousemove",bindLeftSliderMove);
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart").addEventListener("mousemove",bindLeftSliderMove);
    });

    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideLSel").addEventListener("mouseup", function(e){
      e.stopPropagation();
    ...
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideLSel").addEventListener("mouseleave", function(e){
      e.stopPropagation();
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideLSel").removeEventListener("mousemove",bindLeftSliderMove);
    });
    ...
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideRSel").addEventListener("mousedown", function(e){
      e.stopPropagation();
      PAGE_DATA.mouseDown = 1; 
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideRSel").addEventListener("mousemove",bindRightSliderMove);
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart").addEventListener("mousemove",bindRightSliderMove);
    });

    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideRSel").addEventListener("mouseup", function(e){
      e.stopPropagation();
      PAGE_DATA.mouseDown = 0; 
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideRSel").removeEventListener("mousemove",bindRightSliderMove);
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart").removeEventListener("mousemove",bindRightSliderMove);
      resetSliderPos("right",PAGE_DATA.fullSeries[PAGE_DATA.longestSeries][PAGE_DATA.windowRightIndex].x);
      reDrawSeries();
    });

    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideRSel").addEventListener("mouseleave", function(e){
      e.stopPropagation();
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideRSel").removeEventListener("mousemove",bindRightSliderMove);
    });
    ...
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideLSel").addEventListener("touchstart", function(e){
      e.stopPropagation();
      PAGE_DATA.mouseDown = 1; 
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideLSel").addEventListener("touchmove",bindLeftSliderMove);
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart").addEventListener("touchmove",bindLeftSliderMove);
    },false);

    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideLSel").addEventListener("touchend", function(e){
      e.stopPropagation();
      PAGE_DATA.mouseDown = 0; 
      document.querySelector("#"+ PAGE_OPTIONS.targetElem+" #lineChart #slideLSel").removeEventListener("touchmove",bindLeftSliderMove);
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart").removeEventListener("touchmove",bindLeftSliderMove);
      resetSliderPos("left",PAGE_DATA.fullSeries[PAGE_DATA.longestSeries][PAGE_DATA.windowLeftIndex].x);
      reDrawSeries();
    },false);
    
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideRSel").addEventListener("touchstart", function(e){
      e.stopPropagation();
      PAGE_DATA.mouseDown = 1; 
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideRSel").addEventListener("touchmove",bindRightSliderMove);
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart").addEventListener("touchmove",bindRightSliderMove);
    });

    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideRSel").addEventListener("touchend", function(e){
      e.stopPropagation();
      PAGE_DATA.mouseDown = 0; 
      document.querySelector("#"+PAGE_OPTIONS.targetElem+" #lineChart #slideRSel").removeEventListener("touchmove",bindRightSliderMove);
