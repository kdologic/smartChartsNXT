...
window.drawAreaChart = function(opts)
{
  ...
  function prepareChart()
  {
    ...
    strSVG += "<rect id='sliderLeftOffset' x='"+(PAGE_DATA.marginLeft+PAGE_DATA.scaleX)+"' y='"+((PAGE_DATA.svgCenter.y*2)-PAGE_DATA.marginBottom+PAGE_DATA.fullChartHeight)+"' width='0' height='"+(PAGE_DATA.scaleY+60)+"' fill= '#777'  style='stroke-width:0.1;stroke:#717171;' \/>";
    strSVG += "<rect id='sliderRightOffset' x='"+((PAGE_DATA.svgCenter.x*2)-PAGE_DATA.marginRight+PAGE_DATA.scaleX)+"' y='"+((PAGE_DATA.svgCenter.y*2)-PAGE_DATA.marginBottom+PAGE_DATA.fullChartHeight)+"' width='0' height='"+(PAGE_DATA.scaleY+60)+"' fill= '#777' style='stroke-width:0.1;stroke:#717171;' \/>";
    ...
    resetSliderPos("left",(PAGE_DATA.marginLeft+PAGE_DATA.scaleX+(fullSXinterval/2)));
  ...
  function createHorizontalLabel(categories,scaleX)
  {
    ...
    { 
      strText +="<tspan x='"+(PAGE_DATA.marginLeft+(hText*interval)+(interval/2))+"' y='"+(PAGE_DATA.marginTop+PAGE_DATA.gridBoxHeight+20)+"' font-size='12' >"+categories[hText]+"<\/tspan>";
    }
    strText += "</text>";
    for(var hText =0;hText<categories.length;hText++)
    { 
  ...
  function resetTextPositions()
  {
    ...
    console.log(totalHTextWidth/interval);
    if(parseFloat(totalHTextWidth+(arrHText.length*10)) > parseFloat(PAGE_DATA.gridBoxWidth) )
    {
      for(var i=0;i<arrHText.length;i++)
      {
        ...
        arrHText[i].setAttribute("transform","rotate(90 "+cx+","+cy+")");*/
        var cx = arrHText[i].getAttribute("x");
        var cy = arrHText[i].getAttribute("y"); 
        arrHText[i].setAttribute("x",(parseFloat(cx)+11));
        arrHText[i].setAttribute("writing-mode","tb");
      }
    }
  ...
  function bindEvents()
  {
    for(var index=0;index<PAGE_OPTIONS.dataSet.series.length;index++)
    {
      ...
      (function(indx,catList,dataSet){
        ...
        if(seriesArea) seriesArea.addEventListener("mousemove", function(e){
          ...
          if(toolTipPoint && toolTipPoint.onLine1 === true && toolTipPoint.onLine2 === true)
          {
            ...
            if((toolTipPoint.x > (PAGE_DATA.series[indx][nearestPointIndex].x-5)) && (toolTipPoint.x < (PAGE_DATA.series[indx][nearestPointIndex].x+5)) ){
              ...
              document.querySelector("#"+PAGE_OPTIONS.targetElem+" #areaChart #txtToolTipGrp #toolTipYval").textContent = (PAGE_OPTIONS.dataSet.yAxis.title+" "+dataSet[indx][nearestPointIndex]);
            }
            else{
        ...
        if(seriesArea) seriesArea.addEventListener("click", function(e){
  ...
  function resetSliderPos(type,x)
  {
    ...
    if(type === "left"){
      ...
      sliderOffset.setAttribute("width",(x-sliderOffset.getBBox().x));
    }else {
  ...
  function showToolTip(cPoint,color)
  {
    ...
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #areaChart #toolTip").addEventListener("mouseenter",function(){
