...
window.SmartChartsNXT.AreaChart = function(opts)
{
  ...
  function createFullSeries()
  {
    ...
    function drawFullSeries(dataSet,index)
    {
      ...
      if(!PAGE_OPTIONS.smoothedLine && arrPointsSet.length > 1)
  ...
  function createSeries(dataSet,index,scaleX)
  {
    ...
    if(PAGE_OPTIONS.smoothedLine)
      ...
      strSeries +="<path stroke='"+$SC.util.getColor(index)+"' fill='none' d='"+d.join(" ")+"' stroke-width='3' opacity='1'></path>";
    strSeries += "</g>";
    ...
    if(PAGE_OPTIONS.smoothedLine)
      ...
      strSeries +="<path id='area_"+index+"' stroke='none' fill='"+$SC.util.getColor(index)+"' d='"+area.join(" ")+"' stroke-width='1' opacity='0.3'></path>";
    ...
    for(var point = 0;(point+2)<arrPointsSet.length;point++)
    {
      if(dataSet.length <50){
        if(PAGE_OPTIONS.smoothedLine)
          ...
          strSeries += "<circle cx="+arrPointsSet[point+1].x+" cy="+arrPointsSet[point+1].y+" r='5' class='dot' style='fill:"+$SC.util.getColor(index)+"; opacity: 1; stroke-width: 1px;'></circle>";
      }
    }
    strSeries += "</g>";
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #areaChart").insertAdjacentHTML("beforeend",strSeries);
