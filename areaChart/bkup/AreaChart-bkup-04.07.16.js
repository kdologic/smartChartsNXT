...
window.drawAreaChart = function(opts)
{
  ...
  function prepareChart()
  {
    ...
    strSVG = "<text id='hTextSubTitle' font-family='Lato'  x='"+(PAGE_DATA.marginLeft+(PAGE_DATA.gridBoxWidth/2)-30)+"' y='"+(PAGE_DATA.marginTop+PAGE_DATA.gridBoxHeight+55)+"' font-size='18' >"+PAGE_OPTIONS.dataSet.xAxis.title+"<\/text>";
    strSVG +="<text id='vTextSubTitle' font-family='Lato'  x='"+(PAGE_DATA.marginLeft-30)+"' y='"+(PAGE_DATA.marginTop+(PAGE_DATA.gridBoxHeight/2)-5)+"' font-size='18' >"+PAGE_OPTIONS.dataSet.yAxis.title+"<\/text>";
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #areaChart").insertAdjacentHTML("beforeend",strSVG);
  ...
  function showAnimatedView(){
    ...
    var timoutId = setInterval  (function(){
      ...
      if(pointIndex === PAGE_OPTIONS.dataSet.series[PAGE_DATA.longestSeries].data.length){
        ...
        appendMenu("smartCharts-menu-contr");
      }
      pointIndex++;
    },50);
  ...
  function appendMenu(){
    ...
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #smartCharts-menu #smartCharts-menu-icon").addEventListener("click", function(e){
      ...
      if(menuSidePanel.length > 0)
      {
        ...
        strSVG += "  <text x='"+((PAGE_DATA.svgCenter.x*2)-offSetX+25)+"' y='"+(offSetY+32)+"' font-family='Lato' >Download As...</text>";
        
        strSVG += "  <rect class='menu-panel-menu' save-as='jpg' x='"+((PAGE_DATA.svgCenter.x*2)-offSetX)+"' y='"+(2*offSetY)+"' width='"+(offSetX-1)+"' height='"+(offSetY)+"' fill='#fff' stroke-width='0' stroke='#333' \/>";
        ...
        strSVG += "  <text x='"+((PAGE_DATA.svgCenter.x*2)-offSetX+25)+"' y='"+(2*offSetY+32)+"' font-family='Lato' >JPEG</text>";
        strSVG += "  <text x='"+((PAGE_DATA.svgCenter.x*2)-offSetX+25)+"' y='"+(3*offSetY+32)+"' font-family='Lato' >PNG</text>";
        strSVG += "  <text x='"+((PAGE_DATA.svgCenter.x*2)-offSetX+25)+"' y='"+(4*offSetY+32)+"' font-family='Lato' >SVG</text>";
  ...
  function saveAsImage(opts){
    ...
    img.onload = function() {
        ...
        var imgAsURL = (opts.type === "svg")?img.src : canvas.toDataURL("image/"+opts.type);
