 ...
 * @Sample caller code:
  ...
  SmartChartsNXT.ready(function(){
    var areaChart = new SmartChartsNXT.AreaChart(settings);
  });
...
window.SmartChartsNXT.AreaChart = function(opts)
{
  ...
  function createFullSeries()
  {
    ...
    function drawFullSeries(dataSet,index)
    {
      ...
      {
        var p = new $SC.geom.Point(PAGE_DATA.marginLeft+(dataCount*scaleX)+(scaleX/2),(PAGE_DATA.marginTop+PAGE_DATA.gridBoxHeight+PAGE_DATA.fullChartHeight+PAGE_DATA.fcMarginTop)-(dataSet[dataCount]*scaleYfull));
        arrPointsSet.push(p);
      }
  ...
  function createSeries(dataSet,index,scaleX)
  {
    ...
    for(var point = 0;(point+2)<arrPointsSet.length;point++)
    {
      ...
      line.push.apply(line,curve);
      if(dataSet.length <50)
        strSeries += "<circle cx="+arrPointsSet[point+1].x+" cy="+arrPointsSet[point+1].y+" r='3' class='dot' style='fill:"+$SC.util.getColor(index)+"; opacity: 1; stroke-width: 1px;'></circle>";
    }
    line.push.apply(line,["L",arrPointsSet[arrPointsSet.length-1].x,arrPointsSet[arrPointsSet.length-1].y]);
    area.push.apply(area,line);
    ...
    area.push.apply(area,d);  
    strSeries +="<path id='line_"+index+"' stroke='"+$SC.util.getColor(index)+"' fill='none' d='"+line.join(" ")+"' stroke-width='1' opacity='1'></path>";
    strSeries +="<path id='area_"+index+"' stroke='none' fill='url(#"+PAGE_OPTIONS.targetElem+"-areachart-gradLinear"+index+")' d='"+area.join(" ")+"' stroke-width='1' opacity='1'></path>";
  ...
  function resetTextPositions()
  {
    ...
    txtTitleGrp.querySelector("#txtTitle").setAttribute("y",(PAGE_DATA.svgCenter.y-230));
    txtTitleGrp.querySelector("#txtSubtitle").setAttribute("x",(PAGE_DATA.svgCenter.x-(txtSubTitleLen/2)));
    txtTitleGrp.querySelector("#txtSubtitle").setAttribute("y",(PAGE_DATA.svgCenter.y-210));
    ...
    if(parseFloat(totalHTextWidth+(arrHText.length*10)) > parseFloat(PAGE_DATA.gridBoxWidth) )
    {
      for(var i=0;i<arrHText.length;i++)
      {
        ...
        var txtLen = arrHText[i].getComputedTextLength();
        arrHText[i].setAttribute("transform", "translate("+(-txtLen+20)+","+(txtLen-20)+") rotate(-45,"+cx+","+cy+")");
  ...
  function bindEvents()
  {
    for(var index=0;index<PAGE_OPTIONS.dataSet.series.length;index++)
    {
      ...
      (function(indx){
        ...
        if(seriesArea) {
          seriesArea.removeEventListener("mousemove", onMouseMove);
          seriesArea.addEventListener("mousemove", onMouseMove,false);
        }
        
        var legend = document.querySelector("#"+PAGE_OPTIONS.targetElem+" #areaChart #series_legend_"+indx);
        if(legend){
    ...
    try{
      ...
      { 
        ...
        for(var i=0;i<PAGE_DATA.series.length;i++)  {
          if(document.querySelector("#"+PAGE_OPTIONS.targetElem+" #areaChart #area_"+i).style.display === "none")
            continue;
        ...
        if(toolTipPoint)
        {
          ...
          $SC.geom.createDot(toolTipPoint,"#FFF",6,1,"tooTipPoint","areaChart",$SC.util.getColor(indx));
          $SC.geom.createDot(toolTipPoint,$SC.util.getColor(indx),4,1,"tooTipPoint","areaChart");

          var toolTipRow1,toolTipRow2;
          toolTipRow1=(PAGE_OPTIONS.dataSet.xAxis.title+" "+PAGE_DATA.newCatgList[npIndex]);
          toolTipRow2=(PAGE_OPTIONS.dataSet.yAxis.title+" "+(PAGE_OPTIONS.dataSet.yAxis.prefix||"")+" "+PAGE_DATA.newDataSet[indx][npIndex]);
        ...
        $SC.ui.toolTip(PAGE_OPTIONS.targetElem,"hide");
        document.querySelector("#"+PAGE_OPTIONS.targetElem+" #areaChart #vLine").style.display = "none";
  ...
  function resetSliderPos(type,x)
  {
    ...
    document.querySelector("#"+PAGE_OPTIONS.targetElem+" #areaChart #"+sliderSel).setAttribute("d",$SC.geom.describeEllipticalArc(x,cy,15,15,180,360,swipeFlag).d);//("cy",((PAGE_DATA.svgCenter.y*2)-PAGE_DATA.marginBottom+PAGE_DATA.fullChartHeight+(PAGE_DATA.fullChartHeight/2)));
